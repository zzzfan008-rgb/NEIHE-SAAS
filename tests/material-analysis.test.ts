...
const input = await sharp({
  ...
  .png()
  .toBuffer();
const prepared = await cropMaterialImage(
...
const metadata = await sharp(
  Buffer.from(prepared.cropDataUrl.split(",")[1], "base64"),
).metadata();
...
const orientedInput = await sharp({
  ...
  .toBuffer();
const orientedCrop = await cropMaterialImage(
  `data:image/jpeg;base64,${orientedInput.toString("base64")}`,
  {
    x: 0,
    y: 0,
    width: 1,
...
const orientedMetadata = await sharp(
  Buffer.from(orientedCrop.cropDataUrl.split(",")[1], "base64"),
).metadata();
assert.deepEqual(
  { width: orientedMetadata.width, height: orientedMetadata.height },
  { width: 20, height: 40 },
);
console.log("  ✓ EXIF 方向在归一化坐标裁切前应用");

assert.throws(
  () => validateMaterialCrop({ x: 0.8, y: 0, width: 0.3, height: 1 }),
  /超出/,
);
assert.throws(
  () => validateMaterialCrop({ x: 0, y: 0, width: 1, height: 1, extra: true }),
  /未知字段/,
);
await assert.rejects(
  () =>
    cropMaterialImage(`data:image/png;base64,${input.toString("base64")}`, {
      x: 0,
      y: 0,
      width: 0.1,
      height: 0.1,
...
process.env.APIYI_BASE_URL ||= "https://apiyi.example.test";
process.env.APIYI_API_KEY ||= "test-only";

let requestedUrl = "";
let requestedBody = "";
const analyzed = await analyzeMaterialImage(
  prepared.cropDataUrl,
  "gemini-test",
