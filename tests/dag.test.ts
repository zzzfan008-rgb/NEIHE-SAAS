...
async function main() {
  ...
  await ok("runner 分步换装使用角色提示词并固定第二轮中等质量", async () => {
    ...
    assert.equal(stageOne.calls[0].request.referenceImages?.at(-2), POSE_DATA_URL, "原始姿势图必须直接进入生图请求");
    ...
    assert.match(stageOne.calls[0].request.prompt, /参考图11是用户手动选择的原始姿势参考图/);
