...
assert.equal(TOOL_GROUPS.flatMap((group) => group.items).length, 25);
...
assert.deepEqual(item("3D 视角")?.creationIntent, { type: "node", kind: "ti-angle" });
for (const name of [
  "文生视频",
  "首帧生视频",
  "首尾帧生视频",
...
console.log("通过 1 项工具目录测试（5 组、25 项）");
