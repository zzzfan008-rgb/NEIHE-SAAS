...
assert.ok(app.volumes.some(v => v.type === 'volume' && v.source === 'app_data' && v.target === '/app/data'),
  'Production data must be isolated from Git checkout changes');
assert.ok(!app.volumes.some(v => v.type === 'bind' && v.target === '/app/data'));
assert.equal(config.volumes.app_data.name, `${config.name}_app_data`);
...
assert.match(gitIgnored, /^data\/templates\/builtin\/$/m);
const tracked = spawnSync('git', ['ls-files', 'data/templates/builtin'], { encoding: 'utf8' });
assert.equal(tracked.status, 0);
assert.equal(tracked.stdout.trim(), '', 'Generated templates must not be tracked by Git');
