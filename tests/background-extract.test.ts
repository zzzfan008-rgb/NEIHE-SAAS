...
async function main(): Promise<void> {
  ...
  const uploadedPlan = buildExecutionPlan([uploaded], []);
  assert.deepEqual(uploadedPlan.steps[0]?.inputImages, [SOURCE_IMAGE]);
  assert.doesNotThrow(() => assertPlanInputs(uploadedPlan, []));
