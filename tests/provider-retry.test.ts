...
let transportInit: (RequestInit & { dispatcher?: unknown }) | undefined;
globalThis.fetch = (async (_input, init) => {
  transportInit = init as RequestInit & { dispatcher?: unknown };
  return new Response("{}", { status: 200 });
}) as typeof fetch;
try {
  await fetchWithRetry("https://gateway.example/v1/images/generations", () => ({}), {
    providerId: "stub",
    timeoutMs: 60_000,
  });
  assert.ok(transportInit?.dispatcher, "请求必须使用独立的 undici dispatcher");
  assert.ok(transportInit?.signal instanceof AbortSignal, "请求必须保留总时限 AbortSignal");
} finally {
