...
| 检查点 | 实现与结论 |
| --- | --- |
| 路径与方法 | POST `/v1beta/models/gemini-3-pro-image-preview:generateContent`，JSON、服务端 Bearer 认证；符合文档 |
| `parts` | 一个 `{text}` 放首位，之后每张图一个独立 `{inlineData}`；不混合字段、不把 N 图误装入一个图片字段 |
| 图片编码 | `inlineData.data` 是纯 Base64，不含 data-URL 前缀；PNG/JPEG，WebP 转 PNG；符合文档 |
| 输出设置 | `responseModalities: ["IMAGE"]` 合法；宽高比和 1K/2K/4K 由模型设置提供；跟随场景时会按场景比例解析 |
| 返回解析 | 遍历候选 parts，不假定首个 part 是图片；使用最后一张有效终稿，兼容前置文字 |
| 原图排序 | 前三张固定为姿势、人物、场景；服装及配饰在其后，提示词逐图/逐格映射 |
| 六图策略 | Pro 原始有效图 ≤6 时直接传递；>6 时本地拼接，并保持前三图独立；这是用户模板策略，不是 Gemini 接口硬上限 |
| 模型多图能力 | API易多图指南描述最多 14 张模型输入；项目 Flash 也保留 14 图模型校验。Pro 此模板支持最多 20 张原始素材，经拼接最多提交 6 张 |
...
现场日志是 `promptFeedback.blockReason=OTHER`，没有候选图片、候选 token 为 0。原解析器优先把零 token 映射为“安全审核拦截”，依据不足。
...
- 多图测试从执行器到真实适配器截获模拟 HTTP JSON，覆盖 4/5/6/7/20 张原图与普通/简化模式；检查独立 parts、排序、Base64、端点、输出配置与单次图片编辑。
