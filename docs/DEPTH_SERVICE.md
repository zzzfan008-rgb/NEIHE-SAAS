...
对比弹窗提供原图、骨骼图、深度图三栏，支持放大、下载、添加到画布、分别重试和刷新结果。添加图片不会自动连线；用户手动选择一张接入第一轮姿势端口，所选图片原样传递给生图流程。失败不会清除另一侧成功结果。关闭弹窗后任务继续，重新打开只查询状态；服务重启后超时运行记录标记为失败或结果未知，不自动重复付费请求。替换原图或文档后不会回填旧结果。
