...
选中模板的姿势参考图片节点（无需先连线），点击“生成姿势参考”或“查看对比”打开弹窗。可以分别生成骨骼/深度，或点击“生成两种参考”。骨骼使用[本地 DWPose](DWPOSE_SERVICE.md)，深度也只调用本地服务。生成前自动保存发起请求的项目，保存失败不调用模型。只读项目可以查看既有结果，但不能生成或添加图片到画布。

对比弹窗提供原图、骨骼图、深度图三栏，支持放大、下载、添加到画布、分别重试和刷新结果。DWPose 骨骼图可以选择跟随当前图片来源，也可以在深度图成功后选择“深度图”作为 DWPose 的输入；该选择使用服务端保存的私有深度结果，不创建公开文件、不自动添加或连线，并按深度结果指纹隔离骨骼缓存。添加图片不会自动连线；用户手动选择一张接入第一轮姿势端口，所选图片原样传递给生图流程。失败不会清除另一侧成功结果。关闭弹窗后任务继续，重新打开只查询状态；服务重启后超时运行记录标记为失败或结果未知，不自动重复付费请求。替换原图或文档后不会回填旧结果。
