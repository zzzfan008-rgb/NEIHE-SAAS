...
function stringValue(
  ...
  if (value.length > MAX_TEXT_LENGTH)
    fail(path, `must be at most ${MAX_TEXT_LENGTH} characters`);
  return value;
}
...
function imageReference(
  ...
  if (ref.startsWith("data:")) {
    ...
    try {
      ...
      fail(
        path,
        error instanceof Error ? error.message : "invalid image dataURL",
      );
    }
    if (opts?.allowedDataUrlMimes && !opts.allowedDataUrlMimes.includes(mime)) {
      fail(
        path,
        `dataURL MIME must be one of: ${opts.allowedDataUrlMimes.join(", ")}`,
      );
    }
    return ref;
  }
  ...
  if (!isLocalImageReference(ref) && !isRemote && !ASSET_REFERENCE.test(ref)) {
    fail(
      path,
      "must be an image dataURL, local /api/files reference, http(s) URL, or asset:// reference",
    );
  }
  return ref;
}
...
function optionalMaskReference(
  value: unknown,
  path: string,
): string | undefined {
...
function stringArray(
  ...
  return value.map((item, index) =>
    stringValue(item, `${path}[${index}]`, { nonEmpty: true }),
  );
}

function imageReferenceArray(
  value: unknown,
  path: string,
  max = MAX_IMAGE_REFS,
): string[] {
...
function seedanceRemoteMediaReference(value: unknown, path: string): string {
  ...
  if (
    !/^https:\/\//i.test(ref) &&
    !ASSET_REFERENCE.test(ref) &&
    !isLocalMediaReference(ref)
  ) {
    fail(
      path,
      "must be a local media reference, HTTPS URL, or asset:// reference",
    );
  }
  return ref;
}

function mediaReferenceArray(
  value: unknown,
  path: string,
  max = MAX_IMAGE_REFS,
): string[] {
...
function migratedModelFields(
  ...
  if (raw.modelId !== undefined) {
    const modelId =
      raw.modelId === "gemini-3.1-flash-image-preview"
        ? "gemini-3.1-flash-image"
        : raw.modelId;
    return {
      modelId,
      modelOptions:
        raw.modelOptions === undefined && isImageModelId(modelId)
  ...
  switch (kind) {
    ...
    case "ai-styling":
      return {
        ...
        outputImages: [],
        ...raw,
        ...migratedModelFields(kind, raw, "3:4"),
      };
    case "image-input":
    ...
    case "video-generate": {
      ...
      const mode =
        raw.mode === "multi-image-video"
          ? "multimodal-reference"
          : raw.mode === "video-to-video"
            ? "video-edit"
            : SEEDANCE_VIDEO_MODES.includes(raw.mode as never)
              ? raw.mode
              : "text-to-video";
      ...
      const ratio = [
        ...
        "21:9",
        "adaptive",
      ].includes(String(raw.aspectRatio))
        ? raw.aspectRatio
        : "16:9";
      const resolution =
        raw.resolution === "480p"
          ? "480p"
          : raw.resolution === "1080p" || raw.resolution === "4k"
            ? "1080p"
            : "720p";
      const duration =
        raw.seconds === -1 ||
    ...
    case "sketch-to-render":
      return {
        ...
        ...migratedModelFields(
          kind,
          raw,
          typeof raw.aspectRatio === "string" ? raw.aspectRatio : "3:4",
        ),
      };
    case "ai-modify":
      return {
        ...
        ...migratedModelFields(
          kind,
          raw,
          typeof raw.aspectRatio === "string" ? raw.aspectRatio : "1:1",
        ),
      };
    case "fabric-recolor":
      return {
...
function validateModelSelection(
  ...
  if (
    ...
    return;
  if (!isImageModelId(raw.modelId))
    fail(`${path}.modelId`, "must be a supported API易 image model");
  ...
  const normalizedOptions = normalizeImageModelOptions(
    raw.modelId,
    inputOptions,
  );
  ...
  switch (kind) {
    case "outfit-reference": {
      ...
      if (raw.mainImage !== null) {
        ...
        if (!images.includes(main))
          fail(`${path}.mainImage`, "must belong to images");
      }
      if (images.length && raw.mainImage === null)
        fail(`${path}.mainImage`, "must select a main image");
      break;
    }
    case "character-board":
      if (
        raw.sourceImage !== undefined &&
        !isLocalImageReference(raw.sourceImage)
      ) {
        fail(
          `${path}.sourceImage`,
    ...
    case "ai-styling": {
      ...
      if (raw.preserve !== null)
        oneOf(
          raw.preserve,
          ["upper", "lower", "one-piece", "whole"] as const,
          `${path}.preserve`,
        );
      const extras = record(raw.extras, `${path}.extras`);
      for (const key of ["outerwear", "shoes", "bag", "accessories", "hat"])
        booleanValue(extras[key], `${path}.extras.${key}`);
      for (const key of ["analysisId", "referenceFingerprint", "resultNodeId"])
        optionalString(raw[key], `${path}.${key}`);
    ...
    case "image-input":
      ...
      if (raw.autoConnectTargets !== undefined) {
        if (
          ...
          raw.autoConnectTargets.length > MAX_AUTO_CONNECT_TARGETS
        ) {
          fail(
            `${path}.autoConnectTargets`,
            `must be an array with at most ${MAX_AUTO_CONNECT_TARGETS} entries`,
          );
        }
        ...
        raw.autoConnectTargets.forEach((value, index) => {
          ...
          const targetNodeId = stringValue(
            target.targetNodeId,
            `${targetPath}.targetNodeId`,
            { nonEmpty: true },
          );
          if (!SAFE_ID.test(targetNodeId))
            fail(`${targetPath}.targetNodeId`, "must be a valid node id");
          const targetHandle = oneOf(
            target.targetHandle,
            WORKFLOW_INPUT_ROLES,
            `${targetPath}.targetHandle`,
          );
          const key = `${targetNodeId}\u0000${targetHandle}`;
          if (seenTargets.has(key))
            fail(
              targetPath,
              "must not duplicate an automatic connection target",
            );
          seenTargets.add(key);
        });
      }
    ...
    case "drawing-board":
      ...
      break;
    case "color-palette": {
      const paletteVersion = oneOf(
        raw.paletteVersion,
        [1, 2] as const,
        `${path}.paletteVersion`,
      );
      if (
        !Array.isArray(raw.swatches) ||
        raw.swatches.length < 1 ||
        raw.swatches.length > 32
      ) {
      ...
      raw.swatches.forEach((value, index) => {
        ...
        const color = stringValue(swatch.value, `${swatchPath}.value`, {
          nonEmpty: true,
        });
        if (!/^#[0-9A-F]{6}$/.test(color))
          fail(`${swatchPath}.value`, "must be canonical uppercase #RRGGBB");
        optionalString(swatch.name, `${swatchPath}.name`);
        const source = oneOf(
          swatch.source,
          COLOR_SWATCH_SOURCES,
          `${swatchPath}.source`,
        );
        const expectsPantone = source === "pantone" || source === "brand";
        if (paletteVersion === 1 && expectsPantone)
          fail(
            `${swatchPath}.source`,
            "Pantone sources require paletteVersion 2",
          );
        let identityKey = `hex:${color}`;
        if (expectsPantone) {
          const identity = record(swatch.pantone, `${swatchPath}.pantone`);
          const catalogId = stringValue(
            identity.catalogId,
            `${swatchPath}.pantone.catalogId`,
            { nonEmpty: true },
          );
          if (!/^[0-9a-f]{64}$/.test(catalogId))
            fail(
              `${swatchPath}.pantone.catalogId`,
              "must be a 64-character lowercase digest",
            );
          for (const field of ["releaseId", "libraryKey", "code"] as const) {
            stringValue(identity[field], `${swatchPath}.pantone.${field}`, {
              nonEmpty: true,
            });
          }
          ...
          fail(
            `${swatchPath}.pantone`,
            "ordinary colors must not carry Pantone identity",
          );
        }
        if (identities.has(identityKey))
          fail(swatchPath, "identity must be unique");
    ...
    case "stage-approval":
      oneOf(
        raw.approvalKind,
        ["scene-baseline"] as const,
        `${path}.approvalKind`,
      );
      optionalString(raw.approvedSourceNodeId, `${path}.approvedSourceNodeId`);
      optionalImageReference(
        raw.approvedBaselineRef,
        `${path}.approvedBaselineRef`,
      );
      optionalNonNegativeInteger(
        raw.approvedBasisRevision,
        `${path}.approvedBasisRevision`,
      );
      optionalString(raw.approvedAt, `${path}.approvedAt`);
      break;
    case "video-input":
      if (raw.videoUrl !== undefined)
        mediaReference(raw.videoUrl, `${path}.videoUrl`);
      if (raw.mimeType !== undefined)
        oneOf(
          raw.mimeType,
          ["video/mp4", "video/webm", "video/quicktime"] as const,
          `${path}.mimeType`,
        );
      break;
    case "audio-input":
      if (raw.audioUrl !== undefined)
        seedanceRemoteMediaReference(raw.audioUrl, `${path}.audioUrl`);
      if (raw.mimeType !== undefined)
        oneOf(
          raw.mimeType,
          ["audio/mpeg", "audio/wav", "audio/mp4", "audio/ogg"] as const,
          `${path}.mimeType`,
        );
      break;
    case "video-generate":
      ...
      if (
        ...
        fail(
          `${path}.resolution`,
          "1080p is only supported by Seedance 2.5 and 2.0 standard",
        );
      }
      if (
        !SEEDANCE_MODEL_CAPABILITIES[
        ...
        raw.outputFormat !== "mp4"
      ) {
        fail(`${path}.outputFormat`, "mov is only supported by Seedance 2.5");
      }
      if (
        (raw.mode === "video-edit" || raw.mode === "video-extend") &&
        raw.videoModel !== "doubao-seedance-2-5-260628"
      ) {
      ...
      if (
        ...
        fail(
          `${path}.aspectRatio`,
          "must be adaptive for this Seedance 2.5 task type",
        );
      }
    ...
    case "fabric-recolor": {
      ...
      for (let i = 0; i < colors.length; i++) {
        if (!/^#[0-9a-fA-F]{6}$/.test(colors[i]))
          fail(`${path}.colors[${i}]`, "must be #RRGGBB");
      }
      stringValue(raw.prompt, `${path}.prompt`);
    ...
    case "virtual-try-on":
      ...
      stringValue(raw.stylePresetId, `${path}.stylePresetId`, {
        nonEmpty: true,
      });
      ...
      if (
        raw.workflowStage === "garment-refine" &&
        raw.modelId !== "gpt-image-2" &&
        raw.modelId !== "gpt-image-2.5-sunburst"
      ) {
...
function validateEdge(value: unknown, index: number): PersistedWorkflowEdge {
  ...
  if (source === target)
    fail(path, "source and target must be different nodes");
  if (raw.sourceHandle !== undefined && raw.sourceHandle !== null)
...
function migrateMaskRepairEdges(
  nodeValues: unknown[],
  edgeValues: unknown[],
): unknown[] {
  const maskNodeIds = new Set(
    nodeValues.flatMap((value) => {
      if (typeof value !== "object" || value === null || Array.isArray(value))
        return [];
      const node = value as Record<string, unknown>;
      return node.type === "mask-redraw" && typeof node.id === "string"
        ? [node.id]
        : [];
    }),
  );
  ...
  return edgeValues.map((value) => {
    ...
    if (typeof edge.target !== "string" || !maskNodeIds.has(edge.target))
      return value;
...
function recoverStagedTryOnNode(
  ...
  if (typeof value !== "object" || value === null || Array.isArray(value))
    return value;
  const node = value as Record<string, unknown>;
  if (node.type !== "virtual-try-on" || typeof node.id !== "string")
    return value;
  if (
  ...
  if (data.workflowStage !== undefined && data.workflowStage !== "standard")
    return value;
  const roles = incomingRoles.get(node.id);
  if (!roles) return value;

  const inferred =
    (data.modelId === "gemini-3.1-flash-image-preview" ||
    ...
    roles.has("outfit")
      ...
      : (data.modelId === MASK_REDRAW_MODEL_ID ||
            data.modelId === "gpt-image-2") &&
          roles.has("baseline") &&
          roles.has("outfit")
        ? "garment-refine"
        : undefined;
  return inferred
    ? { ...node, data: { ...data, workflowStage: inferred } }
    : value;
}
...
function migrateLegacyDualModelAccessorySlot(
  ...
  const nodeRecords = nodeValues.filter(
  ...
  const nodes = nodeValues.map((node) => {
    ...
    if (record.id === "accessory") {
      return {
        ...record,
        data: { ...data, label: "鞋履参考图（可选）", imageRole: "reference" },
      };
    }
    if (record.id === "structure") {
      return {
        ...record,
        data: { ...data, label: "包袋参考图（可选）", imageRole: "reference" },
      };
    }
    return node;
  });
  ...
  const edges = edgeValues.flatMap((edgeValue): unknown[] => {
    ...
    if (
      ...
      edge.target === "stabilize" &&
      accessoryRoles.has(edge.source)
    ) {
...
function normalizeLegacyEdgeHandles(edgeValues: unknown[]): unknown[] {
  return edgeValues.map((value) => {
    ...
    const targetHandle =
      ...
      WORKFLOW_INPUT_ROLES.includes(edge.targetHandle as WorkflowInputRole)
        ? edge.targetHandle
        : null;
    const sourceHandle =
      typeof edge.sourceHandle === "string" &&
      (["image", "text", "colors", "video", "audio"].includes(
        edge.sourceHandle,
      ) ||
        /^image:\d+$/.test(edge.sourceHandle))
        ? edge.sourceHandle
        : null;
...
function migrateLegacyVideoEdges(
  nodeValues: unknown[],
  edgeValues: unknown[],
): unknown[] {
  const legacyMultimodalIds = new Set(
    nodeValues.flatMap((value) => {
      if (typeof value !== "object" || value === null || Array.isArray(value))
        return [];
      const node = value as Record<string, unknown>;
      if (node.type !== "video-generate" || typeof node.id !== "string")
        return [];
      const data =
        typeof node.data === "object" &&
        node.data !== null &&
  ...
  return edgeValues.map((value) => {
    ...
    return edge.targetHandle === "first-frame" ||
      edge.targetHandle === "last-frame"
...
function migrateV4StageApprovals(
  ...
  const nodes = nodeValues.map((value) => {
    ...
    if (
      ...
      return value;
    const data = node.data as Record<string, unknown>;
    if (node.type === "fabric-recolor")
      return { ...node, data: { operationMode: "combined", ...data } };
    if (
  ...
  for (const [refineId, refineNode] of byId) {
    ...
    if (
      typeof refineNode.data !== "object" ||
      refineNode.data === null ||
      Array.isArray(refineNode.data)
    )
      continue;
    ...
    const candidateEdgeId = stableMigrationId(
      "baseline_candidate",
      legacyEdge.id,
    );
    if (byId.has(approvalId))
      fail("flow.nodes", `migration id collision: ${approvalId}`);
    const sourcePosition =
      typeof sourceNode.position === "object" && sourceNode.position !== null
        ? (sourceNode.position as Record<string, unknown>)
        : {};
    const refinePosition =
      typeof refineNode.position === "object" && refineNode.position !== null
        ? (refineNode.position as Record<string, unknown>)
        : {};
    const sourceData =
      typeof sourceNode.data === "object" && sourceNode.data !== null
        ? (sourceNode.data as Record<string, unknown>)
        : {};
    const sourceOutputs = Array.isArray(sourceData.outputImages)
      ? sourceData.outputImages
      : [];
    const validApprovedRef =
      typeof legacyApprovedRef === "string" &&
      sourceOutputs.includes(legacyApprovedRef)
        ? legacyApprovedRef
        : undefined;
    const approvalNode = {
      id: approvalId,
      ...
      position: {
        x:
          ((typeof sourcePosition.x === "number" ? sourcePosition.x : 0) +
            (typeof refinePosition.x === "number" ? refinePosition.x : 0)) /
          2,
        y:
          ((typeof sourcePosition.y === "number" ? sourcePosition.y : 0) +
            (typeof refinePosition.y === "number" ? refinePosition.y : 0)) /
          2,
      },
      data: {
        kind: "stage-approval",
        ...
        ...(validApprovedRef
          ? {
              ...
              approvedBasisRevision:
                typeof sourceData.basisRevision === "number"
                  ? sourceData.basisRevision
                  : 0,
              ...(typeof legacyApprovedAt === "string"
                ? { approvedAt: legacyApprovedAt }
                : {}),
            }
          : {}),
...
export function validateAndMigrateFlow(value: unknown): PersistedWorkflow {
  ...
  if (!migrateLegacy && version !== WORKFLOW_SCHEMA_VERSION) {
    fail(
      "flow.schemaVersion",
      `unsupported version ${String(version)}; current version is ${WORKFLOW_SCHEMA_VERSION}`,
    );
  }
  ...
  const upgraded = migrateLegacy
    ? {
        ...approvalUpgraded,
        edges: migrateMaskRepairEdges(
          approvalUpgraded.nodes,
          approvalUpgraded.edges,
        ),
      }
    : approvalUpgraded;
  ...
  if (upgraded.edges.length > MAX_EDGES)
    fail("flow.edges", `must contain at most ${MAX_EDGES} edges`);

  const nodes = upgraded.nodes.map((node, index) =>
    validateNode(node, index, migrateLegacy),
  );
  ...
  const typedStagedPairs = new Set(
    recoveredEdges.flatMap((edge) =>
  ...
  const edges = recoveredEdges.filter(
    (edge) =>
      !(
  ...
  for (const node of nodes) {
    if (nodeIds.has(node.id))
      fail("flow.nodes", `duplicate node id: ${node.id}`);
    nodeIds.add(node.id);
  ...
  for (const edge of edges) {
    if (edgeIds.has(edge.id))
      fail("flow.edges", `duplicate edge id: ${edge.id}`);
    edgeIds.add(edge.id);
    if (!nodeIds.has(edge.source))
      fail("flow.edges", `edge ${edge.id} source not found: ${edge.source}`);
    if (!nodeIds.has(edge.target))
      fail("flow.edges", `edge ${edge.id} target not found: ${edge.target}`);
    ...
    const compatibilityError = connectionCompatibilityError({
      ...
      maxSources:
        targetNode.data.kind === "mask-redraw"
          ? MAX_REFERENCE_IMAGES
          : undefined,
    });
    ...
    if (
      ...
      fail(
        "flow.edges",
        `edge ${edge.id}: asset:// 图片素材只能连接 Seedance 视频节点`,
      );
    }
  ...
  for (const node of nodes) {
    ...
    const incomingImageCount = incomingEdges.filter(
      (edge) =>
    ...
    const persistedInputLimit =
      node.type === "mask-redraw"
        ? MAX_REFERENCE_IMAGES
    ...
    if (
      ...
      fail(
        "flow.edges",
        `node ${node.id} accepts at most ${MAX_REFERENCE_IMAGES} reference images`,
      );
    }
  }
  return documentSnapshotToPersistedWorkflow(
    createDocumentSnapshot({
      projectName: "",
      nodes,
      edges,
