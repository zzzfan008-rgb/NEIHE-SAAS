...
export async function assertWorkflowPantoneReferences(
  ...
  const references = flow.nodes.flatMap((node) => {
    if (node.data.kind !== "color-palette" || node.data.paletteVersion !== 2) return [];
    return node.data.swatches.flatMap((swatch) => swatch.pantone ? [{ ...swatch.pantone, hex: swatch.value }] : []);
  });
  if (references.length === 0) return;

  const catalogIds = [...new Set(references.map((reference) => reference.catalogId))];
  const releaseIds = [...new Set(references.map((reference) => reference.releaseId))];
  ...
  `, [catalogIds, releaseIds], client);
  const canonical = new Map(rows.map((row) => [`${row.release_id}:${row.color_id}`, row]));
  for (const reference of references) {
    const key = `${reference.releaseId}:${reference.catalogId}`;
    const row = canonical.get(key);
    if (!row || row.library_key !== reference.libraryKey || row.code !== reference.code || row.hex !== reference.hex) {
      throw new WorkflowValidationError(`Pantone 色号 ${reference.code} 与版本主库不一致`);
    }
  }
}
