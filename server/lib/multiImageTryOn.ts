...
export function multiImageTryOnPrompt(referenceMap: string, extra: string, angleControlled: boolean, concise = false, poseReferenceType?: unknown, posePrompt?: unknown): string {
  ...
  const posePromptClause = typeof posePrompt === "string" && posePrompt.trim()
    ? `用户确认的姿势描述：${posePrompt.trim()}。该描述与图1可见动作几何共同决定最终动作，冲突时以图1可见几何为准。`
    : "";
  return [
    "以参考图2提供的人物造型基调，创作一位原创虚构模特的时尚服装摄影照片，展示下方指定的服装和配饰。人物为原创虚构角色或已获授权的模特形象，不代表、不映射任何真实在世或已故个人、公众人物或知名 IP 角色。",
    skeleton
      ? `动作（最高优先级）：图1是DWPose骨骼图，是唯一姿势锚点。${posePromptClause}按可见关键点与连线1:1复刻人物动作：每个关键点的位置、连线方向与相对比例逐点对齐，包括肩髋倾斜、肘腕与膝踝弯曲、双手手指与双脚朝向、双腿弯曲与前后关系、重心与承重关系；左右以图中画面方向为准，不镜像。本动作约束优先于人物、场景、服装及任何相机视角指令，后续相机视角描述只在其基础上调整观察角度，不得改变上述关节动作。缺失关键点不作为约束，不从线条推断视线、表情、体型或精确前后深度，不自行摆正躯干或拉直四肢。骨骼线条与关键点不得渲染到成图。`
      : `动作（最高优先级）：图1是唯一姿势锚点。${posePromptClause}逐关节1:1复刻图1的动作：身体朝向、头部朝向、肩髋倾斜、肩肘腕与髋膝踝位置、双臂与手部动作、双腿弯曲与前后关系、重心与承重关系；左右以图中画面方向为准，不镜像，不自行摆正躯干或拉直四肢。本动作约束优先于人物、场景、服装及任何相机视角指令，后续相机视角描述只在其基础上调整观察角度，不得改变上述关节动作。图1中的人物身份、五官、面部特征、体型、发型、服装、鞋履、配饰与背景一律禁止进入成片，不得用图1的外貌或身材替代图2的人物。`,
