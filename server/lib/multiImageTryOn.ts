...
export function multiImageTryOnPrompt(referenceMap: string, extra: string, angleControlled: boolean, concise = false): string {
  return [
    concise ? "根据以下参考素材完成服装展示，直接输出完整成片。" : "执行一次多图编辑换装，直接输出完整成片，保留所提供商品的可见细节。",
    "参考图1提供姿势、四肢动作与左右关系；参考图2用于保持参考人物外观一致，包括面部、发型与体型；参考图3提供场景、光照和环境。各图仅用于对应职责。",
    angleControlled ? "拍摄视角与构图遵循下方的3D视角指令；姿势图控制关节动作，但不覆盖3D视角。" : "拍摄视角、透视与构图结合场景和用户要求安排；保持姿势参考的动作与左右关系，不镜像。",
    "按以下逐图、逐格对应关系穿戴服装、鞋袜与配饰：",
    referenceMap,
    concise ? "拼图各格是独立商品参考，不是输出布局。保留可见的版型、颜色、纹理、工艺及配饰细节。" : "拼图只是参考素材目录，不能照搬白底、网格、边框或拼贴布局到成片；各格商品保持独立，商品图人物不作为人物外观参考。",
    concise ? "" : "完整保留原始商品图中可见的版型、颜色、印花、纹理、针织组织、蕾丝、缝线、纽扣、五金、鞋袜结构和配饰形状。不得用概括性设计替代可见细节，不虚构不可见文字或工艺。",
    "只输出一张自然、写实、光照协调的穿搭照片，不输出素材板或对比图。",
