...
export function multiImageTryOnPrompt(referenceMap: string, extra: string, angleControlled: boolean, concise = false, poseReferenceType?: unknown): string {
  ...
  return [
    ...
    skeleton
      ? "动作：图1是DWPose骨骼图，只提供二维动作几何。按可见关键点与连线1:1复刻人物动作：每个关键点的位置、连线方向与相对比例逐点对齐，包括肩髋倾斜、肘腕与膝踝弯曲、双手手指与双脚朝向、双腿弯曲与前后关系、重心与承重关系；左右以图中画面方向为准，不镜像。缺失关键点不作为约束，不从线条推断视线、表情、体型或精确前后深度，不自行摆正躯干或拉直四肢。骨骼线条与关键点不得渲染到成图。"
      : "动作：图1只提供动作几何参考。参照图1提取身体朝向、头部朝向、肩髋倾斜、肩肘腕与髋膝踝位置、双臂与手部动作、双腿弯曲与前后关系、重心与承重关系；左右以图中画面方向为准。图1中的人物身份、五官、面部特征、体型、发型、服装、鞋履、配饰与背景一律禁止进入成片，不得用图1的外貌或身材替代图2的人物。",
    "环境：采用图3的场景、光照和环境色彩，使人物和商品具有协调的光影与透视；不继承场景图中人物的动作、外观或服装。",
    angleControlled
      ? skeleton
        ? "拍摄视角与构图遵循下方的3D视角指令；骨骼图控制关节动作，但不覆盖3D视角。保留关节弯曲、手脚接触与承重关系，按指定相机投影，不把镜头变化理解为改变动作。"
        : "拍摄视角与构图遵循下方的3D视角指令；姿势图控制关节动作，但不覆盖3D视角。保留身体动作、关节弯曲、手脚接触与承重关系，按指定相机投影，不把镜头变化理解为改变动作。"
      : skeleton
