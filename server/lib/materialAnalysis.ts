...
export function validateMaterialCrop(value: unknown): MaterialCropRect {
  ...
  if (
    Object.keys(row).some((key) => !["x", "y", "width", "height"].includes(key))
  ) {
    throw new ProviderError(
      "裁切范围包含未知字段",
      400,
  ...
  const crop = {
    x: finiteRatio(row.x, "x"),
    y: finiteRatio(row.y, "y"),
    width: finiteRatio(row.width, "width"),
    height: finiteRatio(row.height, "height"),
  };
  if (
    crop.width <= 0 ||
    crop.height <= 0 ||
...
export async function cropMaterialImage(
  ...
  const width = Math.min(
    source.width - left,
    Math.max(1, Math.round(source.width * crop.width)),
  );
  const height = Math.min(
    source.height - top,
    Math.max(1, Math.round(source.height * crop.height)),
  );
  ...
  const buffer = await withImageProcessingSlot(() =>
    sharp(source.buffer, {
      animated: false,
      failOn: "error",
...
export function parseMaterialSuggestion(
  value: unknown,
): MaterialAnalysisSuggestion {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new ProviderError(
  ...
  const row = value as Record<string, unknown>;
  if (
    typeof row.materialDescription !== "string" ||
    !row.materialDescription.trim() ||
    row.materialDescription.length > 1_000 ||
    !Array.isArray(row.colors) ||
    row.colors.length < 1 ||
    row.colors.length > 12
  ) {
    throw new ProviderError(
      "材质分析返回字段无效",
      502,
  ...
  const colors = row.colors.map((entry) => {
    ...
    return {
      hex,
      ...(typeof color.name === "string" ? { name: color.name.trim() } : {}),
    };
  });
  return {
    materialDescription: row.materialDescription.trim(),
    observedAttributes: stringArray(
      row.observedAttributes,
      "observedAttributes",
    ),
    uncertainAttributes: stringArray(
      row.uncertainAttributes,
      "uncertainAttributes",
    ),
    colors,
  };
