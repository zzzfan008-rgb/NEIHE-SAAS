...
): ToolItem => ({ id, name, icon, description, availability: "available", creationIntent });
...
): ToolItem => ({ id, name, icon, description, availability: "unavailable", disabledReason });
...
export const TOOL_GROUPS: readonly ToolGroup[] = [
  {
    id: "add", label: "添加节点", icon: "plus", items: [
      ...
      available("character-board", "人物板生成", "user-round", "上传模特图，一键生成四视图人物身份板。", { type: "node", kind: "character-board" }),
  ...
  {
    id: "video", label: "视频制作", icon: "clapperboard", items: [
      ...videoCapabilityToolItems(),
    ],
  },
  {
    id: "create", label: "创作工具", icon: "paintbrush", items: [
      available("drawing-board", "绘画工具", "pencil-ruler", "新建独立空白画板并进行手工绘制。", { type: "drawing-board" }),
      available("color-tool", "色彩工具", "pipette", "快速选择、收藏并输出可连接色板。", { type: "color-palette", swatches: [] }),
    ],
  },
] as const;

export function toolGroupById(id: string | null | undefined): ToolGroup | undefined {
