...
export function TryOnPoseReviewSummary({ executionMeta, stage, reviewMode }: {
  ...
  const disabled = reviewMode === 'disabled' || tryOn?.candidateReviewDisabled === true;
  let summary = disabled
    ? '已禁用 · 本次未执行候选质量与姿势评审'
    : '无法判断 · 未完成有效的独立姿势评审，请人工核对';
  if (!disabled) {
  ...
  return (
    <Card role="region" aria-label="独立姿势评审" size="sm" className="mt-4 rounded-md border border-[var(--gc-border)] bg-[var(--gc-control)] text-[var(--gc-text)] ring-0">
      ...
      <CardContent className="space-y-3 text-[11px] leading-5 break-words">
        ...
        <p className="text-[var(--gc-text-muted)]">{disabled
          ? '本次生成结果不会调用评审模型，也不会产生评审请求。'
          : '模型判断仍需人工确认；质量总分不代表姿势精确一致。左右按画面方向。'}</p>
