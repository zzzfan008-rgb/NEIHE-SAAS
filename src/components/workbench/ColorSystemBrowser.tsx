...
import { ColorCatalogPicker, type CatalogSelection } from "@/components/panels/ColorCatalogPicker";
import { useColorQuery } from "@/lib/colorManagementClient";
import type { BrandColorMember, ColorBrand, ColorGroup, ColorSeries } from "@/types/brandColors";
...
function BrandGroups({
  kind, selectedCatalogIds, remaining, onSelect,
}: ColorSystemBrowserProps & { kind: ColorBrand["kind"] }) {
  const brands = usePublicColorQuery<Page<ColorBrand>>("/brands?limit=100&offset=0");
  ...
  const series = usePublicColorQuery<Page<ColorSeries>>(
    brandId ? `/series?brandId=${encodeURIComponent(brandId)}&limit=100&offset=0` : null,
    revision,
  );
  ...
  const addGroup = () => {
    ...
    if (available.length > remaining) {
      setError(`该主题有 ${available.length} 个未选颜色，当前仅剩 ${remaining} 个名额；请单独选择。`);
      return;
    }
    setError(undefined);
  ...
  if (brands.error) {
    return <p role="alert" className="text-xs text-red-400">{brands.error}</p>;
  }
  if (!brands.data) return <p role="status" className="text-xs">读取品牌色系…</p>;
  if (visibleBrands.length === 0) {
    return <p className="text-xs text-(--gc-text-muted)">暂未配置可用品牌色系。</p>;
  }
    ...
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(11rem,0.7fr)_minmax(0,1.5fr)] gap-3">
      <div className="flex min-h-0 flex-col gap-2">
        {visibleBrands.length > 1 && (
          <Select value={brandId} onValueChange={(value) => { if (value) setBrandId(value); }}>
        ...
        {(series.data?.items.length ?? 0) > 0 && (
          <Select value={seriesId} onValueChange={(value) => { if (value) setSeriesId(value); }}>
            <SelectTrigger aria-label="系列与季节" className="h-8 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent positionerClassName="z-[90]" className="z-[90]">
              <SelectItem value="__all_series__">全部系列</SelectItem>
              {series.data!.items.map((item) => (
                <SelectItem key={item.id} value={item.id}>
                  {[item.year, item.season, item.name].filter(Boolean).join(" · ")}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        )}
        <div className="min-h-0 flex-1 space-y-1 overflow-auto" aria-label="主题色组">
          {groups.error && (
            <div className="space-y-1">
              <p role="alert" className="text-xs text-red-400">{groups.error}</p>
              <Button type="button" size="xs" variant="outline" onClick={() => setRevision((value) => value + 1)}>重试色组</Button>
            </div>
          )}
          {groups.data?.items.map((item) => (
            <Button
              ...
              onClick={() => { setGroupId(item.id); setError(undefined); }}
            >
              {item.name}
            </Button>
          ))}
          {groups.data?.items.length === 0 && <p className="text-xs text-(--gc-text-muted)">暂无主题色组。</p>}
        </div>
        <div className="flex justify-between">
          <Button type="button" size="xs" variant="ghost" disabled={groupOffset === 0}
            onClick={() => setGroupOffset(Math.max(0, groupOffset - 25))}>上一页</Button>
          <Button type="button" size="xs" variant="ghost" disabled={groups.data?.nextOffset == null}
            onClick={() => setGroupOffset(groups.data!.nextOffset!)}>下一页</Button>
        </div>
      </div>
      <div className="flex min-h-0 min-w-0 flex-col gap-2 rounded-md border border-(--gc-border) p-2">
        {!groupId && <p className="text-xs text-(--gc-text-muted)">选择一个主题色组查看颜色。</p>}
        {group.error && <p role="alert" className="text-xs text-red-400">{group.error}</p>}
        {group.data && (
          <>
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <h4 className="truncate text-xs font-medium">{group.data.name}</h4>
                <p className="text-[10px] text-(--gc-text-muted)">{group.data.members.length} 色</p>
              </div>
              <Button type="button" size="xs" variant="outline" disabled={group.data.members.length === 0}
                onClick={addGroup}>整组加入</Button>
            </div>
            <ul className="min-h-0 flex-1 space-y-1 overflow-auto" aria-label={`${group.data.name} 色组颜色`}>
                ...
                return (
                  <li key={member.catalogId}>
                    <Button type="button" variant="outline"
                      ...
                      <span aria-hidden="true" className="size-6 shrink-0 rounded border border-(--gc-border)" style={{ backgroundColor: member.hex }} />
                      <span className="min-w-0 text-left">
                        <span className="block break-all">{member.code}</span>
                        <span className="block break-all text-[10px] text-(--gc-text-muted)">
                          {member.libraryKey} · {member.hex}{member.ratio === null ? "" : ` · ${(member.ratio * 100).toFixed(2)}%`}
                        </span>
                      </span>
                    </Button>
                  </li>
  ...
  return (
    <Tabs defaultValue="catalog" className="flex min-h-[20rem] flex-1 flex-col">
      <TabsList className="grid h-9 w-full grid-cols-3 bg-(--gc-control)">
        <TabsTrigger value="catalog" className="text-xs">全系列</TabsTrigger>
        <TabsTrigger value="neihe" className="text-xs">NEIHE Color</TabsTrigger>
        <TabsTrigger value="reference" className="text-xs">参考品牌</TabsTrigger>
      </TabsList>
