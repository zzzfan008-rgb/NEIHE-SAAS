  ...
  return (
    <>
      <Dialog open onOpenChange={(open) => { if (!open) requestClose(); }}>
        ...
        >
          <DialogTitle>{target.item ? "编辑" : "新建"}{label}</DialogTitle>
          <DialogDescription>保存后对所有用户立即可见；未标注的年份和季节请留空。</DialogDescription>
          <form
            ...
            onSubmit={(event) => {
              ...
              void colorRequest<ColorBrand | ColorSeries>(
                ...
                .catch((reason: unknown) => {
                  ...
                  setConflict(reason instanceof ColorRequestError && reason.status === 409);
                })
                .finally(() => setBusy(false));
            }}
          >
            <label className="block text-xs">{label}名称
              <Input required maxLength={120} value={name} disabled={busy || conflict || outcomeUnknown}
                onChange={(event) => setName(event.target.value)} className="mt-1" />
            </label>
            {target.kind === "series" && (
              <div className="grid grid-cols-2 gap-3">
                <label className="text-xs">年份
                  <Input type="number" min={1900} max={2200} value={year} disabled={busy || conflict || outcomeUnknown}
                    onChange={(event) => setYear(event.target.value)} />
                </label>
                <label className="text-xs">季节
                  <Input maxLength={80} value={season} disabled={busy || conflict || outcomeUnknown}
            ...
            <div className="flex justify-end gap-2">
              {(conflict || outcomeUnknown) && (
                <Button variant="outline" type="button" disabled={busy} onClick={onRefresh}>关闭并刷新目录</Button>
              )}
              <Button variant="outline" type="button" disabled={busy} onClick={requestClose}>取消</Button>
              <Button type="submit" disabled={busy || conflict || outcomeUnknown}>{busy ? "保存中…" : "保存"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
      <AlertDialog open={discardOpen} onOpenChange={setDiscardOpen}>
        <AlertDialogContent overlayClassName="z-90" className="z-90 bg-(--gc-panel) text-(--gc-text)">
          <AlertDialogTitle>放弃未保存的{label}修改？</AlertDialogTitle>
          <AlertDialogDescription>
            关闭将丢弃当前表单内容。
            {outcomeUnknown && <span className="mt-2 block">上一次保存结果可能未知；离开后请刷新目录核对，避免重复创建。</span>}
          </AlertDialogDescription>
          <AlertDialogFooter>
            <AlertDialogCancel>继续编辑</AlertDialogCancel>
            <AlertDialogAction variant="destructive" onClick={onClose}>放弃修改</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
