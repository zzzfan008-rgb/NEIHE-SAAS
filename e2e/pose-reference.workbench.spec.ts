...
async function setup(page: Page) {
  ...
  await page.evaluate(async()=>{
    ...
    useFlowStore.getState().loadFlow({projectName:'姿势对比验收',nodes:[
      {id:'pose-test',type:'image-input',position:{x:170,y:140},data:{kind:'image-input',label:'人物姿势参考图（必需）',status:'success',imageRole:'reference',imageUrl:'/api/files/pose-source.png'}},
    ...
    ],edges:[{id:'pose-edge',source:'pose-test',target:'target',targetHandle:'pose'}]});
...
test('pose comparison persists, preserves partial results and fits three desktop widths',async({page},testInfo)=>{
  ...
  await page.route('**/api/pose-references**',async route=>{
    ...
    records[body.kind]={id:body.kind,kind:body.kind,source:body.source,status:body.kind==='skeleton'&&!body.retry?'failed':'succeeded',...(body.kind==='skeleton'&&!body.retry?{error:'骨骼分析未成功'}:{result:{image,model:'test-model'}})};
  ...
  await dialog.getByRole('button',{name:'重试骨骼图（可能计费）'}).click();
