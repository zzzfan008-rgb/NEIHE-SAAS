...
test("提取背景：移除原图后切换上游，运行中禁止移除", async ({ page }) => {
  await page.goto("/");
  ...
  await page.getByRole("menuitem", { name: /提取背景/ }).click();
  ...
  await node.getByLabel("上传原图").setInputFiles({ name: "source.png", mimeType: "image/png", buffer: image });
  await expect(node.getByAltText("待提取的原图")).toBeVisible();
  const source = await node.getByAltText("待提取的原图").getAttribute("src");
  for (const status of ["queued", "running", "retry_wait", "cancel_requested", "idle"]) {
    ...
    if (status !== "idle") await expect(node.getByRole("button", { name: "移除原图" })).toBeDisabled();
  }
  await page.evaluate(async () => {
    const path = "/src/store/flowStore.ts";
    const m = await import(path);
  ...
  await expect(node.getByRole("button", { name: "提取背景", exact: true })).toBeDisabled();
  await node.getByRole("button", { name: "移除原图" }).click();
  await expect(node.getByAltText("待提取的原图")).toHaveCount(0);
  await expect(node.getByRole("button", { name: "提取背景", exact: true })).toBeEnabled();
