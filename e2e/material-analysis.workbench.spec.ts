...
const draft = (
  revision: number,
  status: "draft" | "analyzed" | "saved" | "outcome_unknown",
): MaterialAnalysisRecord => ({
  id: "material01",
  status,
  revision,
  sourceImage: "/api/files/source-private.png",
  cropImage: "/api/files/crop-private.png",
  crop: { x: 0.1, y: 0.1, width: 0.8, height: 0.8 },
  modelId: status === "draft" ? null : "gemini-default",
  suggestion:
    status === "draft"
      ? null
      : {
  ...
  await page.route(
    ...
    async (route: Route) => {
      ...
      if (url.pathname.endsWith("/analyze")) {
        analyzeCount += 1;
        current = draft(current.revision + 1, "analyzed");
        await route.fulfill({ json: current });
        return;
      }
      if (url.pathname.endsWith("/save")) {
        savedCalibration = (
          route.request().postDataJSON() as { calibration: unknown }
        ).calibration;
        ...
        assets = [
          {
            id: "material-asset-1",
            name: "团队斜纹面料",
            image: "/api/files/crop-private.png",
            category: "fabric",
            scope: "shared",
            ownerId: "admin",
            canManage: true,
            createdAt: "2026-09-15T00:00:00.000Z",
            material: {
              analysisId: "material01",
              modelId: "gemini-default",
              crop: current.crop,
              materialDescription: current.calibration?.materialDescription,
              colors: current.calibration?.colors,
              analyzedAt: current.updatedAt,
...
test("fabric upload is cropped, reanalyzed, calibrated and saved as a shared asset", async ({
  ...
  await expect(
    dialog.getByRole("button", { name: "保存为共享面料" }),
  ).toBeDisabled();
  ...
  await expect(
    page
      .getByRole("dialog", { name: "资产库", exact: true })
      .getByText("团队斜纹面料"),
  ).toBeVisible();
  const reopenedLibrary = page.getByRole("dialog", {
    name: "资产库",
    exact: true,
  });
  await reopenedLibrary
    .getByRole("button", { name: "查看面料校准信息 团队斜纹面料" })
    .click();
  const details = page.getByRole("dialog", {
    name: "面料校准信息",
    exact: true,
  });
  await expect(details.getByLabel("面料材质描述")).toHaveValue(
    "无法确定成分；可见细密斜纹",
  );
  ...
  const detailsDiscard = page.getByRole("alertdialog", {
    name: "放弃未保存的面料校准修改？",
  });
  ...
  await expect
    .poll(() => api.materialPatch)
    .toMatchObject({
      name: "团队斜纹面料",
      material: {
        materialDescription: "人工复核：细密斜纹",
        colors: [{ hex: "#AABBCC" }],
      },
    });
  const libraryAfterEdit = page.getByRole("dialog", {
    name: "资产库",
    exact: true,
  });
  await libraryAfterEdit
    .getByRole("button", { name: "查看面料校准信息 历史面料" })
    .click();
  const legacyDetails = page.getByRole("dialog", {
    name: "面料校准信息",
    exact: true,
  });
  await expect(
    legacyDetails.getByText("该历史面料暂无分析校准信息。"),
  ).toBeVisible();
  await expect(
    legacyDetails.getByRole("button", { name: "保存校准信息" }),
  ).toHaveCount(0);
  await legacyDetails.getByRole("button", { name: "关闭" }).click();
});

test("refresh preserves human calibration and unknown analysis requires confirmed retry", async ({
  page,
}) => {
  const api = await installMaterialApi(page);
  const library = await openAssetLibrary(page);
  ...
  const dialog = page.getByRole("dialog", { name: "上传并分析面料" });
  const png = await sharp({
    create: { width: 30, height: 30, channels: 3, background: "#778899" },
  })
    .png()
    .toBuffer();
  await dialog
  ...
  api.setCurrent({
    ...draft(3, "analyzed"),
    suggestion: {
      ...draft(3, "analyzed").suggestion!,
      materialDescription: "刷新后的模型描述",
      colors: [{ hex: "#FFFFFF" }],
    },
  });
...
test("changing or closing an unsaved analysis requires explicit server-draft deletion", async ({
  page,
}) => {
  const api = await installMaterialApi(page);
  const library = await openAssetLibrary(page);
  ...
  const dialog = page.getByRole("dialog", { name: "上传并分析面料" });
  const png = await sharp({
    create: { width: 40, height: 40, channels: 3, background: "#8899AA" },
  })
    .png()
    .toBuffer();
  ...
  await expect(
    page.getByRole("dialog", { name: "资产库", exact: true }),
  ).toBeVisible();
  expect(api.deleteCount).toBe(1);
});
test("admin can revise the model allowlist with optimistic revision", async ({
  page,
}) => {
  ...
  const discardModels = page.getByRole("alertdialog", {
    name: "放弃未保存的色彩管理修改？",
  });
...
test("unknown model writes lock editing and closing until server reconciliation", async ({
  page,
}) => {
  await page.route("**/api/material-analyses/models", async (route: Route) => {
    if (route.request().method() === "PUT") {
  ...
  await expect(
    dialog.getByRole("button", { name: "保存模型配置" }),
  ).toBeDisabled();
  await page.keyboard.press("Escape");
  await expect(dialog).toBeVisible();
